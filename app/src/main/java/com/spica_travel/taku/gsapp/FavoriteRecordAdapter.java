...
class FavoriteRecordsAdapter extends ArrayAdapter<FavoriteRecord> {
    ...
    public FavoriteRecordsAdapter(Context context) {
        ...
        fImageLoader = new ImageLoader(VolleyApplication.getInstance().getRequestQueue(), new BitmapLruCache());
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        FavoriteRecord imageRecord = getItem(position);

        //mImageLoaderを使って画像をダウンロードし、Viewにセットします。
        imageView.setImageUrl(imageRecord.getImageUrl(), fImageLoader);
        //Viewに文字をセットします。
...
//        textView3.setText(imageRecord.getComment());
    ...
    //データをセットしなおす関数
    public void setFavoriteRecords(List<FavoriteRecord> objects) {
        //ArrayAdapterを空にする。
        clear();
        //テータの数だけMessageRecordを追加します。
        for(FavoriteRecord object : objects) {
            add(object);
        }
        //データの変更を通知します。
